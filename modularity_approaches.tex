...
We are interested in  \emph{logic-based modules}, for which  relevance amounts  to entailment (or model) preservation  over a signature $\Sigma$. %The latter acts as an interface for communication with the ontology in question. 
 ...
 Observe that the logical view appears to be theoretically sound and elegant and guarantees that by reusing only terms from $\Sigma$ one is not able to distinguish between importing $\Mmc$ and importing \Omc into some ontology $\Pmc'$. However, the decision problem associated to conservative extension is  difficult in general: deciding whether \Omc is a $\Sigma$-conservative extension of $\Omc'$ is 2NEXPTIME-complete for $\ALCQI$ (roughly OWL-Lite) and undecidable for OWL-DL. For model conservative extensions the problem is highly undecidable (non recursively enumerable), even for \ALC.  These computational obstacles have lead, e.g., to approximations via locality.  Locality-based modules provide all logical guaranties of CE-based modules, and  although they are not minimal, they  have been shown to be useful for economically reusing ontologies.   
...
%\item 
\end{enumerate}
Note that the notion of model $\Sigma$-conservative extension is strictly stronger than the deductive  one and it does not depend on the expressiveness of the language $L$. That is $\Omc$ being a $\Sigma$-mCE of \Mmc implies that \Omc is a $\Sigma$-dCE of \Mmc. 

\smallskip\noindent
A more general notion to characterize  modules is that  of \emph{(model-theoretic) inseparability}.
Let $\Omc_1$ and $\Omc_2$ be ontologies and $\Sigma$ a signature. Then $\Omc_1$ and $\Omc_2$ are \emph{ $\Sigma$-model inseparable}, written $\Omc_1 \equiv^m_{\Sigma} \Omc_2$ if, $\{\Imc|_{\Sigma} : \Imc \models \Omc_1\} = \{\Imc|_\Sigma : \Imc \models \Omc_2\}$.  $\Sigma$-model inseparability provides a very strong form of equivalence as $\Omc_1 \equiv^m_\Sigma \Omc_2$ guarantees that $\Omc_1$ can be replaced with $\Omc_2$ in any application that refers only to symbols from $\Sigma$. Indeed, $\Omc_1 \equiv^m_\Sigma \Omc_2$ if and only if 
$$\Omc_1 \models \varphi \text{ iff } \Omc_2 \models \varphi$$
holds for any second order sentence $\varphi$ using only symbols from $\Sigma$.  
%
Weaker versions of inseparability relations can be defined. For a given logic $L$, an \emph{inseparability relation} is a family $S = \{\equiv^S_\Sigma \mid \Sigma  \text{ is a signature}\}$ of equivalence relations on the set of $L$ theories. The intuition behind this notion is as follows: $\Omc_1 \equiv^S_\Sigma \Omc_2$  means that $\Omc_1$ and $\Omc_2$ are indistinguishable w.r.t.\ $\Sigma$, i.e., they represent the same knowledge about the topic represented by $\Sigma$. The precise definition of the inseparability relation determines the exact meaning of the terms ``indistinguishable" and ``the same knowledge". $\Mmc$ being a module for $\Sigma$ of \Omc should be equivalent to $\Mmc \subseteq \Omc$ and $\Mmc$ being inseparable w.r.t.\ $\Sigma$ from \Omc. Under the requirement to preserve entailments, the following inseparability relations can be defined (in the context of description logics):
%
\begin{itemize}
\item $\Omc_1$ and $\Omc_2$ are $\Sigma$-\emph{concept inseparable}, in symbols $\Omc_1 \equiv^c_\Sigma \Omc_2$, if for all  concept names $A,B$ in $\Sigma$, it holds that $\Omc_1 \models A \sqsubseteq B$ if and only if $\Omc_2 \models A \sqsubseteq B$.
\item  $\Omc_1$ and $\Omc_2$ are $\Sigma$-\emph{subsumption inseparable} w.r.t.\ a logic $L$, written $\Omc_1 \equiv^s_\Sigma \Omc_2 $, if for all concept descriptions  $C,D$ in $L$ over $\Sigma$, it holds that $\Omc_1 \models C \sqsubseteq D$ if and only if $ \Omc_2 \models C \sqsubseteq D$.   
\end{itemize}
%
%%%%%%%%%
It is easy to see that, for each signature $\Sigma$, it holds that $\equiv^m_\Sigma \ \subseteq \ \equiv^s_\Sigma \ \subseteq \ \equiv^c_\Sigma$. 

\subsection*{Module Notions}
Conservative extension and inseparability relations induce modules as follows.  Let  %$L$ be a logic,
 \Omc be  an ontology, $\Mmc \subseteq \Omc$ and $\Sigma$ a signature, and $S$\footnote{e.g., $S \in \{m, s, c\}$} an inseparability relation. We call \Mmc

\begin{enumerate}
%\item a \emph{dCE-based (mCE-based) module} for $\Sigma$ of \Omc if \Omc is a $\Sigma$-dCE ($\Sigma$-mCE) of \Mmc w.r.t. $L$
\item a \emph{$\Sigma$-module} of \Omc induced by $S$ if $\Mmc \equiv^S_\Sigma \Omc$; 
%\footnote{analogous notions can be define  for other inseparability relations}
\item a \emph{self-contained $\Sigma$-module} of \Omc  induced by $S$ if $\Mmc  \equiv_{\Sigma \cup \mn{Sig}(\Mmc)} \Omc$;
\item a \emph{depleting $\Sigma$-module}  of \Omc  induced by $S$  if $  \Omc \setminus \Mmc \equiv_{\Sigma \cup \mn{Sig}(\Mmc)} \emptyset$
\end{enumerate}
Modules induced by model-theoretic inseparability are  mCE-based modules: for a given signature $\Sigma$,  if $\Mmc \subseteq \Omc$ and $\Mmc \equiv^m_\Sigma \Omc$, then $\Omc$ is a $\Sigma$ mCE of  \Mmc.  In the remaining of this paper  we will focus on modules induced by model-theoretic inseparability. 

We say that a  subset $\Mmc \subseteq \Omc$ \emph{covers} all the knowledge that $\Omc$ has about $\Sigma$, if for every axiom $\alpha$  with, $\mn{Sig}(\alpha) \subseteq \Sigma$,  we have that $\Omc \models \alpha$ if and only if $\Mmc \models \alpha$.  Note that in that case   $\Omc$ is a $\Sigma$ dCE of $\Mmc$.  
...
A ``plain" $\Sigma$-module \Mmc of \Omc covers all knowledge that \Omc has about $\Sigma$.
%
...
In contrast to ``plain'' $\Sigma$-modules,   for a \emph{depleting} $\Sigma$-module \Mmc of \Omc,  the difference $\Omc \setminus \Mmc$ has no knowledge about $\Sigma \cup \mn{Sig}(\Mmc)$. 
%
This  means that the difference of \Omc and its module \Mmc does not entail
any axioms in terms of $\Sigma \cup  \mn{Sig}(\Mmc)$ other than tautologies. The relationship between the different kinds of modules induced by inseparability relations is as follows. %[Kontchavov et al.,IJCAI09] [Konev et al., JAI 2013]
\begin{itemize}
\item If \Mmc is a self-contained $\Sigma$-module of \Omc, then it is a (plain) $\Sigma$-module.
\item If \Mmc is a depleting $\Sigma$-module of \Omc, then it is a self-contained $\Sigma$-module. 
\end{itemize}
We say that \Omc is \emph{safe} for $\Sigma$ if, for every ontology $\Omc'$ with $\mn{Sig}(\Omc)\cap \mn{Sig}(\Omc') \subseteq \Sigma$, we have that $\Omc \cup \Omc'$ is a model $\Sigma$-conservative extension of $\Omc'$; alternatively, this notion can be formulated in terms of inseparability: an ontology \Omc is safe for a signature $\Sigma$  if and only if $\Omc \equiv^m_\Sigma \emptyset$.
...
\begin{tabular}{ccccc} \toprule 
 \multirow{2}{*}{\bf Properties} &\multicolumn{4}{c}{\bf Module Notions}\\ \cline{2-5}
   & plain    & self-contained & depleting  & 
   %$\Delta, \emptyset, \top, \bot, (\top\bot)^*$ and $(\bot\top)^*$ 
    locality based\\
   %&		&			&	&based		\\ 
   \midrule
 {\bf mCE (dCE)}& \checkmark & \checkmark & \checkmark
 %\multicolumn{4}{c}{\Omc is a mCE (dCE) of \Mmc}
 \\ \midrule
 {\bf inseparability}  & $\Omc \equiv^m_\Sigma \Mmc$ 
   & $\Omc \equiv^m_{\Sigma \cup \mn{Sig}(\Mmc)} \Mmc$ &  $\Omc \setminus \Mmc \equiv^m_{\Sigma \cup \mn{Sig}(\Mmc)}\emptyset$ \\ 
   \midrule
   {\bf self-contained} & \XSolidBrush & \checkmark & \checkmark & \checkmark  \\ \midrule
   {\bf depleting}  &\XSolidBrush & \XSolidBrush & \checkmark &\checkmark \\ \midrule
   {\bf robustness voc. restr.} & $\checkmark$ & \checkmark & \checkmark &\checkmark \\    \midrule
   {\bf robustness voc. ext.} & \checkmark & \checkmark &\checkmark &\checkmark \\ \midrule 
    {\bf robustness replacement} & \checkmark & \checkmark & \checkmark &\checkmark \\ \midrule
   {\bf robustness joins} & \checkmark& \checkmark & \checkmark & \checkmark  \\ \midrule
...
Due to the computational difficulty to decide both kinds of CEs, approximations have been introduced based on the notion of \emph{locality} of  single axioms. Intuitively,  given $\Sigma$, a local axiom can always be satisfied independently of the interpretation of the $\Sigma$-terms, but in a restricted way:  by interpreting all non-$\Sigma$ terms as either the empty set ($\emptyset$-locality) or as the full domain ($\Delta$-locality). In other words, \emph{an axiom $\alpha$ is local w.r.t.\ $\Sigma$} if every trivial expansion of any $\Sigma$-interpretation to $\Sigma \cup \mn{Sig}(\alpha)$ is a model of $\alpha$.
%We omit here the precise definition of local axiom. Instead, we note that 

It has been shown that for   a subset $\Mmc \subseteq \Omc$  all axioms in $\Omc \setminus \Mmc$ being  $\emptyset$-local (or all axioms being $\Delta$-local) w.r.t.\ $\Sigma \cup \mn{sig}(\Mmc)$  is sufficient for $\Omc$ to be a $\Sigma$-mCE of \Mmc. However, the converse does not hold. The previous result can  also be formulated in a more general way as follows. Let $\Omc_1, \Omc_2$ be two ontologies and $\Sigma$ a signature such that $\Omc_2$ is local w.r.t.\ $\Sigma \cup \mn{Sig}(\Omc_1)$,  then $\Omc_1 \cup \Omc_2$ is a $\Sigma$-model conservative extension of $\Omc_1$. 

One can use the standard capabilities of available DL-reasoners for testing $\emptyset$-locality.  Note, however, that  reasoning on expressive DLs (underlying OWL-DL) is not tractable. Moreover, testing for $\Delta$-locality cannot be done using available reasoners, since it requires the test exceed the expressivity  OWL-DL. 
In order to achieve \emph{tractable} module extraction,  syntactic approximations of locality have been introduced.   
%
...
%\includegraphics[scale=.7]{locality}
    \begin{algorithm}
       \begin{algorithmic}
         \vspace*{2pt}
         \STATE{\textbf{Input:}~  TBox \Tmc,~ signature $\Sigma$},~ $x \in \{\emptyset,\Delta,\bot,\top\}$
         \STATE{\textbf{Output:}~ $x$-module \Mmc of \Omc w.r.t.\ $\Sigma$}
         \vspace*{4pt}
         \hrule
         \vspace*{4pt}
         \STATE{$M \leftarrow \emptyset$;~ $\Tmc' \leftarrow \Tmc$}
         \REPEAT
           \STATE{changed $\leftarrow$ \texttt{false}}
           \FORALL{$\alpha \in \Tmc'$}
             \vspace*{2pt}
             \IF{$\alpha$ not $x$-local w.r.t.\ $\Sigma \cup \mn{Sig}(\Mmc)$}
               \STATE{$\Mmc \leftarrow \Mmc \cup \{\alpha\}$}
               \STATE{$\Tmc' \leftarrow \Tmc' \setminus \{\alpha\}$}
               \STATE{changed $\leftarrow$ \texttt{true}}
             \ENDIF
           \ENDFOR
         \UNTIL{changed $=$ \texttt{false}}
         \STATE{\textbf{return} \Mmc}
       \end{algorithmic}
       \caption{Extract a locality-based module}
       \label{alg:x-module}
     \end{algorithm}
...
\medskip\noindent
In general, locality based  modules  are not minimal but provide the preservation of certain entailments.  Modules based on syntactic locality can be made small by nesting $\top$-extraction into $\bot$-extraction and vice versa, and the result is still an mCE-based module. 
...
$x$-locality ($x \in \{\Delta, \emptyset, \top, \bot\}$) %and $\bot$-locality are sufficient 
is sufficient for safety: 
 If an ontology \Omc is $x$-local 
 %or $\top$-local w.r.t.\  $\Sigma$, 
 then \Omc is safe for $\Sigma$. 
...
For rather unexpressive ontology languages language specific  (minimal) module extraction procedures have been developed. 
...
short, $\Sigma$-reachable) if, for all $x \in  \mn{Sig}(\beta_L)$, $x$ is $\Sigma$-reachable. The \emph{reachability-based module for $\Sigma$ in \Omc} is the set of all $\Sigma$-reachable axioms.
...
It has been shown that for an $\EL^+$ ontology \Omc and signature $\Sigma$, the subset $\Omc_\Sigma \subseteq \Omc$ of all $\Sigma$-reachable axioms in $\Omc$ can be computed in linear time and that it is the minimal $\bot$ locality-based module w.r.t. $\Sigma$ of \Omc.

 \paragraph{Minimal Module Extraction (MEX and AMEX algorithms).} Based on the notion of \emph{depleting $\Sigma$-module}. 
Recall that a depleting $\Sigma$-module can be defined as a subset of an ontology 
%that is self-contained in the sense 
that it is $\Sigma$-inseparable from the ontology, where $\Sigma$ is the signature of the module. 

One of the main features of model theoretic $\Sigma$-inseparability is that it is language independent. 
...
\begin{itemize} 
\item
Deciding $\Sigma$-inseparability is undecidable for \ALC-ontologies even if one ontology is acyclic and the other is empty;
...
$\Omc \setminus \Mmc \equiv^m_{\Sigma \cup \mn{Sig}(\Mmc)} \emptyset$

 then $\Omc \setminus \Mmc$ is safe for $\Sigma \cup \mn{Sig}(\Mmc)$ w.r.t.\ \EL 
\item Equivalently,
$\Mmc \equiv^m_{\Sigma \cup \mn{Sig}(\Mmc)} \Omc$.
%
\end{itemize}
...
\begin{algorithm}
       \begin{algorithmic}
         \vspace*{2pt}
         \STATE{\textbf{Input:}~  TBox \Tmc,~ signature $\Sigma$}
         \STATE{\textbf{Output:}~ \Mmc}
         \vspace*{4pt}
         \hrule
         \vspace*{4pt}
         \STATE{$\Mmc := \emptyset$;}
         \STATE{$ \Wmc:= \emptyset$;}
           \WHILE{$(\Tmc \setminus \Mmc) \neq \Wmc$}
             \vspace*{2pt}
               \STATE{choose $\alpha \in (\Tmc \setminus \Mmc) \setminus \Wmc$;}
               \STATE{$\Wmc := \Wmc \cup \alpha$;}
             \IF{ $\Wmc \not \equiv^m_{\Sigma\cup \mn{Sig}(\Mmc)} \emptyset$}
              \STATE{$\Mmc:= \Mmc \cup \{\alpha\}$};
              \STATE{$\Wmc := \emptyset$}
             \ENDIF
           \ENDWHILE
       %  \UNTIL{changed $=$ \texttt{false}}
        % \STATE{\textbf{return} \Mmc}
       \end{algorithmic}
       \caption{Extracting depleting $\Sigma$-modules}
       \label{alg:x-module}
     \end{algorithm}
...
In this section we summarize the  module extraction algorithms implementations for OWL ontologies discussed.
...
\end{itemize}
 %\\
 %&   & 
 There is also a web-base tool for module extraction that can be found at 
 %Web Module extractor (
 \url{http://mowl-power.cs.man.ac.uk:8080/modularity/}
 %}) \\ \midrule
%    &  &  & Web module extractor  \\
%Module for a concept.   & \\
%model theoretic inseparabiliy & & &\\
\item[MEX.] The minimal module extraction implemented in MEX works for OWL-EL acyclic ontologies.  This tool is  implemented in OCaml as a command line software, and can be found at  \url{http://cgi.csc.liv.ac.uk/~konev/software/mex.bin}
\item[AMEX.] This tool implements minimal module extraction for acyclic $\ALCQI$ Ontologies, which correspond roughly to  acyclic OWL-lite ontologies. The command line tool can be found at \url{http://cgi.csc.liv.ac.uk/~wgatens/software/amex.html}, it is written in Java, built with the OWL-API and distributed as a self-contained java jar archive. It requires the 3rd party QBF solver called sKizzo (\url{http://www.skizzo.info/}) for certain parts of the extraction algorithm (see the AMEX web page for more details).
\end{description}
%\\ \bottomrule
%\end{tabular}
%\end{sideways}
...
However, such approaches do no take into consideration the underlying semantics of the ontology, and hence \emph{do not } generate modules that cover  all the information \emph{relevant} to the elements of $\Sigma$. 
