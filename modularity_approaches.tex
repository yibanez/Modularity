...
\end{enumerate}
Observe that modules induced by model-theoretic inseparability are  mCE-based modules: for a given signature $\Sigma$,  if $\Mmc \subseteq \Omc$ and $\Mmc \equiv^m_\Sigma \Omc$, then $\Omc$ is a $\Sigma$ mCE of  \Mmc.  In the remaining of this paper  we will focus on modules induced by model-theoretic inseparability. 
