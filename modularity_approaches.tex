...
%
We say that \Omc is \emph{safe} for $\Sigma$ if, for every ontology $\Omc'$ with $\mn{Sig}(\Omc)\cap \mn{Sig}(\Omc') \subseteq \Sigma$, we have that $\Omc \cup \Omc'$ is a conservative extension of $\Omc'$; alternatively, this notion can be formulated in terms of inseparability: an ontology \Omc is safe for a signature $\Sigma$  if and only if $\Tmc \equiv^m_\Sigma \emptyset$.
